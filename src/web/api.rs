...
impl ApiService {
    ...
    async fn handle_api_rojo(&self) -> Response<Body> {
        ...
        msgpack_ok(&ServerInfoResponse {
            ...
            workspace: self.serve_session.workspace_path().map(ToOwned::to_owned),
            task_id: self.serve_session.task_id().map(ToOwned::to_owned),
            public_base_url: self.serve_session.public_base_url().map(ToOwned::to_owned),
            expected_place_ids,
