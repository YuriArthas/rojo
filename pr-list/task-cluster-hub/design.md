...
- 为 Linux server cluster 和 Windows 插件自拼域名提供一致的服务身份信息。
...
- 为 Rojo 的状态/摘要接口增加 task 化元数据，至少包含：
  ...
  - `public_base_url`
  - `started_at_unix_ms`
- 状态接口继续提供 `place_id/game_id/session_id` 等现有信息，避免上层丢失校验能力。
...
- helper 返回单个 `base_url` 给插件。
- 但 Rojo 插件看到的服务元数据必须能表达“当前连接属于哪个 task”。
- 插件按 `task_id` 自拼域名时，也能确认当前连接目标。

### 4. 最新收口共识

- Rojo 继续只承担数据面职责，不持有 hub 控制面状态。
- helper 才是本机 Rojo 插件的权威配置源；Rojo 插件不应演化成控制面参与者。
- 插件持有 helper 返回的 task 身份用于一次连接，但不把 `generation` 作为长期缓存主键。
- Rojo 插件向 helper 要当前有效配置，而不是自行理解 task 任期流转。
- 不往 Rojo 插件侧增加新的控制面 token 或会话 id；现有字段暴露范围保持最小。

## 当前实现

- 已支持 task 化 route metadata 注入和读取。
- 已配合 platform 的动态路由和 workspace 校验。
- 当前 helper/插件链路维持单实例假设。

### Windows 接力指引

- Windows 侧重点查看：
  - 本文档的“3. 插件/helper 配置”“4. 最新收口共识”
